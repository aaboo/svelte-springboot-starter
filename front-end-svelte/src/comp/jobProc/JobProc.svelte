...
$:if($menuStore.currentMenuCID==ID){
    NV.action.selectJob({...paramFromSearch[ID]});
...
$:if($menuStore.currentMenuCID==ID){
    ...
    else if(NV.param.workdate!=(new Date()).print("yyyymmdd")) NV.socket.close();
    else NV.socket.connect();
}
...
NV.socket = {
    ...
    , connect: function(){
        const ds = this;
        if(ds.target==null && NV.param.workdate==(new Date()).print("yyyymmdd")){
    ...
    selectJob: function(param){
        ...
        .then(async (response)=>{
            ...
            if(response.data.jobList.length>0){
                ...
                //오늘 날짜조회일 경우 웹소켓 가동
                if(NV.socket.target==null && param.workdate==(new Date()).print("yyyymmdd")){                    
                     NV.socket.connect();
                }else{
                    NV.socket.close();
                }
...
NV.tab = {
    ...
    , list: [
        ...
        , {jobgroup:"PS", jobgroupName:"TAB1", dom:null}
        , {jobgroup:"PR", jobgroupName:"TAB2", dom:null}
        , {jobgroup:"DS", jobgroupName:"TAB3", dom:null}
        , {jobgroup:"DR", jobgroupName:"TAB4", dom:null}
        , {jobgroup:"ET", jobgroupName:"TAB5", dom:null}
    ]
    , obj:{}
    , init: ()=>{
        const ds = NV.tab;
    ...
    <div id="content">
        ...
        <div id="leftSide" bind:this={NV.dom.lside} style="width:{gap.orgX}px">
            <div id="leftSideDesription">            
                ...
                <div id="leftSideDesriptionCenter">  
                </div>
