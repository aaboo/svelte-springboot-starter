...
const NV = {
    ...
    , action: {
        search: ()=>{
            const param = {
                makedate: NV.dom.workdate.value.replace(/[^\d]/g,'')
            }
            if(param.workdate==""){alert('작업월을 입력해 주세요'); return;}
            app.ss.setSearch(param);
            dispatch('search', { paramFromSearch: {...param} }); //조회 발생
        }
        //작업일구분
        , dategubun: (workdate)=>{
...
<div id="jobCtrl_search" class="search">
    <span class="title">{@html icon.search()}&nbsp;메뉴5 조회</span>
    <label>
